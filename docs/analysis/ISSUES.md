...
|---|---|---|---|---|---|---|---|---|
| **S1** | 接入 facets 批量分布（`/select/logsql/facets`） | 功能/API | 新功能 | 🔴 | M | `api/logs.js`(`queryFacets` 已实现无调用方)、`stores/fields.js` | — | ⏸️ |
...
| D22 | 自定义时间无回显/无相对绝对切换 → 回显区间 + 相对/绝对模式 + 快捷区间 | 🟠 | M | — |
...
| D49 | limit 仅客户端截断、无超限提示与续拉 → 真实命中总数比对提示截断 + 时间游标「加载更多」 | 🟠 | M | Q2/D28 |
...
> 路线图衔接见报告「功能更新路线图建议」：v2.1 止损与基础体验 → v2.2–v2.3 分析能力升级 → v3.0 平台化重构。
